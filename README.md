...
* #* **Please run tests from non presentation layer targets on the mac device** *#
...
* MVVM was chosen because it allows us to easily separate responsibilities.
...
* To separate navigation from presentation Coordinators can be added.
* A sixth layer CorePresentation could be added. It would sit in between the Feature and Presentation layer. It would house modules such as CoreUIKit which will contain extensions to UIKit. Same could be done for Watch, and SwiftUI.
...
* There are so many other things in the actual code that could be improved i.e. moving functions reusable code extensions or helpers or things putting items such as `UIView+Accessibility` into the appropriate layer modules.
* Commits, there is a section in branch `P1-00003` where the commit messages change in style and miss the storyID. This is a function of context switching from day job commits to evening work. I would improve renaming the commit messages in a rebase. 
* The `Main` branch should be renamed to `master`
...
When when the feed API call succeeds 
...
Given that as user I have opened the app and the feed is loading 
When when the feed API call succeeds and returns usable data 
Then the loading indicator disappears
...
Given that as user I have opened the app and the feed is loading 
When when the feed API call succeeds but returns no usable data 
Then the loading indicator disappears
